...
console.log(
  `[after update] count = ${(await Agent.get(agent, (s) => s)).count}`,
);
...
console.log(
  `[get_and_update] old count = ${oldCount}, new count = ${(await Agent.get(agent, (s) => s)).count}`,
);
...
console.log(
  `[after cast] double = ${(await Agent.get(agent, (s) => s)).count}`,
);
