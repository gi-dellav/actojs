...
const insert = db.prepare('INSERT INTO items (name, price) VALUES ($name, $price)');
...
insert.run({ $name: 'Gadget', $price: 19.50 });
...
const workerPid = Process.spawn(async () => {
  ...
  deleteById.run({ $id: 1 });
  await Agent.update(writerAgent, state => ({ count: state.count - 1 }));
  console.log(`[worker] deleted id=1, agent count now: ${await Agent.get(writerAgent, s => s.count)}`);
...
const readerPid = Process.spawn(async () => {
  ...
  const row2 = selectById.get({ $id: 2 }) as { id: number; name: string; price: number } | null;
  console.log(`[reader] row id=2 → ${row2 ? JSON.stringify(row2) : 'not found'}`);

  const row99 = selectById.get({ $id: 99 }) as { id: number; name: string; price: number } | null;
  console.log(`[reader] row id=99 → ${row99 ? JSON.stringify(row99) : 'not found'}`);

  // Transaction via actor proxy
  const insertMany = db.transaction((items: { $name: string; $price: number }[]) => {
    let count = 0;
  ...
  await Agent.update(writerAgent, state => ({ count: state.count + inserted }));
  console.log(`[reader] transaction inserted ${inserted} rows`);
});

await Process.sleep(1000);

console.log(`[main] final agent count = ${await Agent.get(writerAgent, s => s.count)}`);
