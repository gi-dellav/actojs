  ...
  start_link() {
    const pid = Process.spawn(async () => {
      ...
      for (;;) {
        ...
        } else if (msg && typeof msg === 'object' && (msg as any).type === 'stop') {
...
console.log(`[main] children: ${counts2.specs} specs, ${counts2.active} active`);

// List children details
const children = await Supervisor.which_children(sup);
for (const c of children) {
  console.log(`[main]   child=${c.id} pid=${c.pid} alive=${Process.alive(c.pid)}`);
}
