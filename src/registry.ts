...
function resolveKeyMode(
  mode: RegistryKeyMode,
): "unique" | "duplicate_key" | "duplicate_pid" {
  if (mode === "unique") return "unique";
  if (mode === "duplicate") return "duplicate_key";
  if (typeof mode === "object" && "duplicate" in mode) {
    if ((mode as { duplicate: string }).duplicate === "key")
      return "duplicate_key";
    if ((mode as { duplicate: string }).duplicate === "pid")
  ...
  return GS.start_link<RegistryState>(
    {
      ...
      handle_call(msg: unknown, from: From, s: RegistryState, myPid: PID) {
        ...
        if (type === "unregister") {
          ...
          return {
            reply:
              before !== entries.length ? undefined : { error: "not_found" },
            state: s,
          };
        }
        ...
        if (type === "lookup") {
          ...
          return {
            reply: entries.map((e) => ({ pid: e.pid, value: e.value })),
            state: s,
          };
        }

        if (type === "match") {
          const { key, pattern, guards } = payload as {
            key: string;
            pattern: unknown;
          ...
          return {
            reply: matched.map((e) => ({ pid: e.pid, value: e.value })),
            state: s,
          };
        }
...
function notifyListeners(
  ...
  for (const listenerName of s.listeners) {
    ...
    if (listenerPid) {
      Proc.send(listenerPid, {
        __registry_event__: true,
        event,
        key,
        pid,
        value,
...
export function register(
  ...
  return GS.call(
    reg,
    { type: "register", payload: { key, value } },
    timeout,
  ) as Promise<{ ok: PID } | { error: string }>;
}
...
export function lookup(
  ...
  return GS.call(reg, { type: "lookup", payload: { key } }, timeout) as Promise<
    { pid: PID; value: unknown }[]
  >;
}
...
export function match(
  ...
  return GS.call(
    reg,
    { type: "match", payload: { key, pattern, guards } },
    timeout,
  ) as Promise<{ pid: PID; value: unknown }[]>;
}
...
export function dispatch(
  ...
  return GS.call(
    reg,
    { type: "dispatch", payload: { key, callback, opts } },
    timeout,
  ) as Promise<void>;
}
...
export function keys(reg: PID, pid: PID, timeout?: number): Promise<string[]> {
  return GS.call(reg, { type: "keys", payload: { pid } }, timeout) as Promise<
    string[]
  >;
}
...
export function update_value(
  ...
  return GS.call(
    reg,
    { type: "update_value", payload: { key, fn } },
    timeout,
  ) as Promise<{ newValue: unknown; oldValue: unknown } | { error: string }>;
}
