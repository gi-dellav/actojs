  ...
  if (rt.spawnProcess) {
    ...
    if (pid != null) {
      ...
      if (caller) {
        ...
        if (callerProc) {
          const found = Array.from(callerProc.monitors.entries()).find(
            ([, t]) => t === pid,
          );
...
export function demonitor(ref: Ref): void {
  ...
  if (monitoredPid) {
    ...
    if (other) {
      const refs = (other.monitoredBy.get(caller) ?? []).filter(
        (r) => r !== ref,
      );
...
export function receive(timeout?: number): Promise<unknown> {
  const caller = M.getCurrentPid();
  if (!caller)
    return Promise.reject(new Error("receive() called outside of a process"));
  if (timeout != null) {
