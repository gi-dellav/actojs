...
interface SubscribeMsg   { action: 'subscribe';   topic: string; pid: PID }
interface UnsubscribeMsg { action: 'unsubscribe'; topic: string; pid: PID }
interface PublishMsg     { action: 'publish';     topic: string; message: unknown }
interface SubscribersMsg { action: 'subscribers'; topic: string }
interface TopicsMsg      { action: 'topics' }

type PubSubCast = SubscribeMsg | UnsubscribeMsg | PublishMsg;

// ---- state ----------------------------------------------------------------

interface MonData { pid: PID; topics: Set<string> }
...
function doSubscribe(state: PubSubState, topic: string, pid: PID, self: PID): void {
  ...
  state.monitors.forEach((mon) => {
    if (!found && mon.pid === pid) { mon.topics.add(topic); found = true; }
  });
  if (!found) {
...
function doUnsubscribe(state: PubSubState, topic: string, pid: PID, self: PID): void {
...
export async function start_link(options?: PubSubOptions): Promise<PID> {
  const result = await GS.start_link(callbacks, undefined, { name: options?.name });
...
export function subscribe(pubsub: PID | string, topic: string): void {
  ...
  GS.cast(pid, { action: 'subscribe', topic, pid: Proc.self() } satisfies SubscribeMsg);
}

/** Unsubscribe the calling process from a topic. */
export function unsubscribe(pubsub: PID | string, topic: string): void {
  const pid = resolve(pubsub);
  GS.cast(pid, { action: 'unsubscribe', topic, pid: Proc.self() } satisfies UnsubscribeMsg);
}

/** Publish a message to a topic. All subscribers receive a tagged message. */
export function publish(pubsub: PID | string, topic: string, message: unknown): void {
  const pid = resolve(pubsub);
  Proc.send(pid, {
    __gen_server__: 'cast',
...
export async function subscribers(
  ...
  const pid = resolve(pubsub);
  return GS.call(pid, { action: 'subscribers', topic } satisfies SubscribersMsg, timeout) as Promise<PID[]>;
}
...
export async function topics(
  ...
  const pid = resolve(pubsub);
  return GS.call(pid, { action: 'topics' } satisfies TopicsMsg, timeout) as Promise<string[]>;
}
