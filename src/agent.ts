  ...
  return startFn<S>(
    {
      ...
      handle_call(msg: unknown, _from: From, s: S, _myPid: PID) {
        ...
        if (type === "get") {
          const fn =
            typeof payload === "function"
              ? (payload as AgentFn<S, unknown>)
              : resolveFn<S, unknown>(payload as AgentArg<S, unknown>);
          const result = fn(s);
        ...
        if (type === "update") {
          const fn =
            typeof payload === "function"
              ? (payload as AgentFn<S, S>)
              : resolveFn<S, S>(payload as AgentArg<S, S>);
          const newState = fn(s);
        ...
        if (type === "get_and_update") {
          const fn =
            typeof payload === "function"
              ? (payload as AgentFn<S, [unknown, S]>)
              : resolveFn<S, [unknown, S]>(
                  payload as AgentArg<S, [unknown, S]>,
                );
...
export async function get_and_update<S, R>(
  ...
  return GS.call(
    pid,
    { type: "get_and_update", payload: fn },
    timeout,
  ) as Promise<R>;
}
