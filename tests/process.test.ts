...
describe('process', () => {
  describe('spawn', () => {
    ...
    test('catches sync errors and process exits', async () => {
      const pid = Process.spawn(() => { throw new Error('boom'); });
    ...
    test('process exits after async function completes', async () => {
      let done = false;
      const pid = Process.spawn(async () => { done = true; });
      await waitFor(() => done);
      await waitFor(() => !Process.alive(pid));
      expect(Process.alive(pid)).toBe(false);
    });
  ...
  describe('alive', () => {
    ...
    test('returns false for exited process', async () => {
      let pid = '';
      Process.spawn(() => { pid = Process.self(); });
      await waitFor(() => pid !== '');
      await waitFor(() => !Process.alive(pid));
      expect(Process.alive(pid)).toBe(false);
    });
  ...
  describe('link / unlink', () => {
    ...
    test('unlink throws outside process', () => {
      M.clearPidStack();
      expect(() => Process.unlink('#PID<0.0.0>')).toThrow('outside of a process');
    });
  ...
  describe('monitor / demonitor', () => {
    test('monitor throws outside process', () => {
      M.clearPidStack();
      expect(() => Process.monitor('#PID<0.0.0>')).toThrow('outside of a process');
    });

    test('demonitor throws outside process', () => {
      M.clearPidStack();
      expect(() => Process.demonitor(Symbol('ref'))).toThrow('outside of a process');
    });
  ...
  describe('flag', () => {
    test('flag throws outside process', () => {
      M.clearPidStack();
      expect(() => Process.flag('trap_exit', true)).toThrow('outside of a process');
    });
  ...
  describe('register / unregister / whereis', () => {
    test('register throws outside process', () => {
      M.clearPidStack();
      expect(() => Process.register('#PID<0.0.0>', 'name')).toThrow('outside of a process');
    });
