...
describe('registry', () => {
  describe('start_link', () => {
    ...
    test('with name option', async () => {
      const result = await Registry.start_link({ keys: 'unique', name: 'my_reg' });
      expect('ok' in result).toBe(true);
      expect(Process.whereis('my_reg')).toBe((result as any).ok);
    });

    test('with partitions', async () => {
      const result = await Registry.start_link({ keys: 'unique', partitions: 4 });
      expect('ok' in result).toBe(true);
    });
  });
  ...
  describe('register / lookup (unique mode)', () => {
    ...
    test('registering duplicate key fails in unique mode', async () => {
      ...
      if ('error' in dupResult) expect(dupResult.error).toBe('already_registered');
    });
  });
  ...
  describe('dispatch', () => {
    ...
    test('dispatch respects limit', async () => {
      ...
      const dispatched: any[] = [];
      await Registry.dispatch(reg, 'q', (_pid, value) => {
        dispatched.push(value);
      }, { limit: 2 });
  ...
  describe('update_value', () => {
    test('updates value for registered key', async () => {
      ...
      const updResult = await Registry.update_value(reg, 'counter', (v: any) => (v as number) + 1);
      M.popPid();
