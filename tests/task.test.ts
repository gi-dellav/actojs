...
describe("task", () => {
  ...
  describe("await_", () => {
    ...
    test("rejects if task not found", async () => {
      await expect(
        Task.await_({ pid: "#PID<999.0.0>", ref: Symbol("x") }),
  ...
  describe("yield_", () => {
    ...
    test("returns null for unknown task", async () => {
      const result = await Task.yield_({
        pid: "#PID<999.0.0>",
        ref: Symbol("x"),
      });
  ...
  describe("start / start_link", () => {
    ...
    test("Task.start_link links the task to the caller", async () => {
      ...
      const result = Task.start_link(async () => {
        await sleep(500);
      });
      expect("ok" in result).toBe(true);
