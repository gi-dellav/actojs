...
describe('TaskSupervisor', () => {
  ...
  describe('async', () => {
    ...
    test('links the task to the caller (behavioural)', async () => {
      ...
      const handle = await TaskSupervisor.async(sup, async () => { await sleep(500); });
      M.popPid();
  ...
  describe('async_nolink', () => {
    test('starts a task without linking to caller', async () => {
      ...
      const handle = await TaskSupervisor.async_nolink(sup, async () => { await sleep(500); });
      M.popPid();
  ...
  describe('start_child', () => {
    test('starts a fire-and-forget task', async () => {
      const { ok: sup } = await TaskSupervisor.start_link();
      let ran = false;
      const result = await TaskSupervisor.start_child(sup, async () => { ran = true; });
      expect('ok' in result).toBe(true);
      await sleep(20);
    ...
    test('child appears in children()', async () => {
      const { ok: sup } = await TaskSupervisor.start_link();
      const { ok: pid } = await TaskSupervisor.start_child(sup, async () => { await sleep(500); });
  ...
  describe('children', () => {
    ...
    test('returns list of PIDs', async () => {
      const { ok: sup } = await TaskSupervisor.start_link();
      const { ok: pid1 } = await TaskSupervisor.start_child(sup, async () => { await sleep(500); });
      const { ok: pid2 } = await TaskSupervisor.start_child(sup, async () => { await sleep(500); });
      const kids = await TaskSupervisor.children(sup);
  ...
  describe('count_children', () => {
    test('returns counts', async () => {
      const { ok: sup } = await TaskSupervisor.start_link();
      await TaskSupervisor.start_child(sup, async () => { await sleep(500); });
      const counts = await TaskSupervisor.count_children(sup);
